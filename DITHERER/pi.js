...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		/* =============================================================================
		   ...
		   + NEW: Blur (px) + Bloom Strength sliders (both applied BEFORE dithering)
		   + NEW: Upload Image (local drive / mobile photos) with Fit/Opacity/Z-order
		   + NEW: Image Zoom slider for uploaded picture (10%–500%)
		   - Paste this into Frame 1 (Actions) of an HTML5 Canvas doc in Adobe Animate.
		   ============================================================================= */
		(function () {
		  // ---------------- Core params (sliders control these) ----------------
		  var ENABLE_DITHER = true;
		  var RETRO_SCALE = 0.22;        // 0.05..1 (internal retro downscale before quantize)
		  var DITHER_INTENSITY = 0.50;   // 0..1.5 (how strong the Bayer offset is)
		  var COLOR_COUNT = 32;          // 2..256 (palette size)
		
		  var HUE_DEG = 0;               // -180..180
		  var SAT_SCALE = 1.0;           // 0..3
		  var CONTRAST_PCT = 0;          // -100..100
		
		  // NEW: Image-space FX before dithering
		  var BLUR_RADIUS = 0;           // 0..12 px (retro buffer pixels)
		  var BLOOM_STRENGTH = 0.00;     // 0..1.5 (0 = off); bright-pass + blur then add
		  ...
		  // ---------------- Upload image state ----------------
		  var __uploadBmp = null;           // createjs.Bitmap instance
		  var __uploadImg = null;           // HTMLImageElement (source)
		  var __uploadFit = "Contain";      // "Contain" | "Cover"
		  var __uploadZ = "Behind";         // "Behind" | "Front"
		  var __uploadOpacity = 1.0;        // 0..1
		  var __uploadZoom = 1.0;           // 0.1..5.0 (multiplier applied after fit)  << NEW
		  var __uploadLayer = new createjs.Container(); // Layer container for flexibility
		  ...
		  function luma(r,g,b){return 0.2126*r + 0.7152*g + 0.0722*b;} // for bloom threshold
		  ...
		  var CUSTOM_PALETTE = false; // becomes true once user edits/extends palette
		  ...
		  function applyBloomToData(data, w, h, strength, blurRadius){
		    ...
		    var threshold = 200; // fixed bright-pass threshold
		  ...
		  function applyDither(stage){
		    if(!ENABLE_DITHER) return;
		    var canvas=stage.canvas; if(!canvas) return;
		    ...
		    // 3) DITHERING (LAST): Bayer threshold + nearest palette quantization
		  ...
		  if(!stage.__ditherWrapped){
		    ...
		    // Insert upload layer default Z at back
		    if (!__uploadLayer.parent) stage.addChildAt(__uploadLayer, 0);
		
		    var _u=stage.update;
		    stage.update=function(){
		      // Ensure upload layer Z-order each tick in case user changed it
		      syncUploadZOrder();
		      _u.apply(stage,arguments);
		      try{applyDither(stage);}catch(e){}
		    };
		  }
		  ...
		  function buildPanel(){
		    ...
		    // Default live display width to current stage width (clamped to 100..2000)
		    try{
		      var cw = stage && stage.canvas ? (stage.canvas.clientWidth || stage.canvas.width) : 720;
		      DISPLAY_WIDTH = clamp(cw, 100, 9000);
		    ...
		    // Exporters use the LIVE display width so output matches what you see
		    ...
		    ui.displayW = SR("Display Width (px)", 100, 9000, 10, DISPLAY_WIDTH, v=>(v|0)+" px", function(v){
		      DISPLAY_WIDTH = clamp(v|0, 100, 9000);
		      applyDisplayWidth(DISPLAY_WIDTH);
		    });
		    ...
		    // Colors slider respects custom palette
		    ...
		    fileInput.capture = "environment"; // mobile: allow camera/photos
		    ...
		    fitSel.value=__uploadFit;
		    fitSel.onchange=function(){ __uploadFit=fitSel.value; layoutUploadBitmap(); };
		    ...
		    zSel.value=__uploadZ;
		    zSel.onchange=function(){ __uploadZ=zSel.value; syncUploadZOrder(); };
		    ...
		    // ---------- NEW: Image Zoom slider ----------
		    var zoomRow = document.createElement("div"); zoomRow.style.display="flex"; zoomRow.style.alignItems="center"; zoomRow.style.gap="12px";
		    var zoomLabel = document.createElement("div"); zoomLabel.textContent="Image Zoom"; zoomLabel.style.fontSize="40px"; zoomLabel.style.minWidth="200px"; zoomLabel.style.fontWeight="800";
		    var zoomSlider = document.createElement("input"); zoomSlider.type="range"; zoomSlider.min="0.1"; zoomSlider.max="5.0"; zoomSlider.step="0.01"; zoomSlider.value=__uploadZoom;
		    zoomSlider.style.width="600px"; zoomSlider.style.height="48px";
		    var zoomVal = document.createElement("div"); zoomVal.textContent=Math.round(__uploadZoom*100)+"%"; zoomVal.style.fontSize="40px"; zoomVal.style.minWidth="120px"; zoomVal.style.textAlign="right";
		    zoomSlider.oninput=function(){ __uploadZoom = clamp(parseFloat(zoomSlider.value)||1, 0.1, 5.0); zoomVal.textContent=Math.round(__uploadZoom*100)+"%"; layoutUploadBitmap(); };
		    ...
		    // Zoom row (same column span as opacity)
		    var zoomWrap=document.createElement("div"); zoomWrap.style.gridColumn="1 / span 3"; zoomWrap.appendChild(zoomRow); upWrap.appendChild(zoomWrap);
		  ...
		  function setUploadBitmapFromImage(imgEl){
		    ...
		    syncUploadZOrder();
		    layoutUploadBitmap();
		    // Force a tick to see changes immediately
		    stage.update();
		  }
		  ...
		  function layoutUploadBitmap(){
		    ...
		    var baseScale;
		    if (__uploadFit === "Contain"){
		      baseScale = Math.min(cw/iw, ch/ih);
		    } else { // Cover
		      baseScale = Math.max(cw/iw, ch/ih);
		    }
		    var scale = baseScale * clamp(__uploadZoom, 0.1, 5.0);   // << APPLY ZOOM
		    __uploadBmp.scaleX = __uploadBmp.scaleY = scale;
		    __uploadBmp.x = (cw - iw*scale)/2;
		    __uploadBmp.y = (ch - ih*scale)/2;
		  }
		
		  function applyUploadOpacity(){
		    if (__uploadBmp) __uploadBmp.alpha = clamp(__uploadOpacity, 0, 1);
		  }
		
		  function syncUploadZOrder(){
		    if (!stage || !__uploadLayer) return;
		    var desiredIndex = (__uploadZ === "Behind") ? 0 : stage.numChildren; // back or top
		    // Ensure upload layer exists exactly once
		    if (!__uploadLayer.parent){
		  ...
		  function startGifSave(durationSec,fps,widthPx,btn,prog){
		    ...
		    widthPx=clamp(widthPx|0,100,9000);
		  ...
		  // Re-layout upload image if window/canvas CSS size changes
...
lib.properties = {
	...
	opacity: 1.00,
	manifest: [
		{src:"images/pi_atlas_1.png?1758166055366", id:"pi_atlas_1"}
	],
	preloads: []
};
