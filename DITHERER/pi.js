...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		/* =============================================================================
		   FRAME 1 — HUD now mounts into Animate's overlay so it's visible with
		   Center Stage + Fit in View. Adds a top-right HUD toggle button.
		   (Includes: Dither pipeline, Breathing FX, Export, Palette, Upload Image)
		   ============================================================================= */
		(function () {
		  // ---------- Params ----------
		  var ENABLE_DITHER = true;
		  var RETRO_SCALE = 0.22, DITHER_INTENSITY = 0.5, COLOR_COUNT = 32;
		  var HUE_DEG = 0, SAT_SCALE = 1.0, CONTRAST_PCT = 0;
		  var BLUR_RADIUS = 0, BLOOM_STRENGTH = 0.0;
		  ...
		  // ---------- Root mount helper (handles Animate overlay) ----------
		  function getHudMount(){
		    ...
		    // Ensure our children can receive input
		    // (pointer-events on parent isn't inherited, but we set this just in case)
		    try { if (root.id === "dom_overlay_container") { root.style.pointerEvents = "auto"; } } catch(_){}
		    // Make sure we're above canvas
		  ...
		  function mountEl(el){
		    var m = getHudMount();
		    // If parent is transformed, position:fixed can behave like absolute in some browsers
		  ...
		  function styleBtn(btn,label,bg){ btn.textContent=label; btn.style.font="900 44px -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif";
		  ...
		  function buildPaletteGrid(){
		    ...
		    var wrap=document.createElement("div"); wrap.style.marginTop="28px";
		    ...
		    var grid=document.createElement("div"); grid.style.display="grid"; grid.style.gridTemplateColumns="repeat(auto-fill, minmax(60px, 1fr))";
		    grid.style.gap="10px"; grid.style.maxHeight="280px"; grid.style.overflow="auto"; grid.style.padding="10px";
		    grid.style.background="rgba(255,255,255,0.05)"; grid.style.borderRadius="12px";
		    (PALETTE||[]).forEach(function(rgb, idx){ var sw=document.createElement("div");
		      sw.style.width="60px"; sw.style.height="60px"; sw.style.borderRadius="10px"; sw.style.boxShadow="inset 0 0 0 2px rgba(255,255,255,.25)";
		      sw.style.background=rgbToHex(rgb[0],rgb[1],rgb[2]); sw.style.cursor="pointer"; sw.title="Index "+idx+" (click to edit)";
		      sw.onclick=function(e){ e.stopPropagation(); colorEditorSwatchIdx=idx; openColorEditor(rgbToHex(rgb[0],rgb[1],rgb[2]),
		      ...
		      grid.appendChild(sw); });
		    // Add tile
		    var add=document.createElement("div"); add.style.width="60px"; add.style.height="60px"; add.style.borderRadius="10px";
		    ...
		    add.style.cursor="pointer"; add.style.fontSize="46px"; add.style.fontWeight="900"; add.textContent="+";
		  ...
		  function openColorEditor(initialHex,onReplace,onAdd){
		    ...
		    box.style.width="820px"; box.style.background="#111"; box.style.color="#fff"; box.style.borderRadius="20px"; box.style.boxShadow="0 20px 100px rgba(0,0,0,.7)";
		    box.style.padding="28px"; box.style.font="28px/1.4 -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif"; box.onclick=function(e){e.stopPropagation();};
		    var title=document.createElement("div"); title.textContent="🎨 Color Editor"; title.style.fontSize="36px"; title.style.fontWeight="900"; title.style.marginBottom="18px"; box.appendChild(title);
		    var row=document.createElement("div"); row.style.display="flex"; row.style.alignItems="center"; row.style.gap="20px"; row.style.marginBottom="18px";
		    var input=document.createElement("input"); input.type="color"; input.value=initialHex||"#ffffff"; input.style.width="160px"; input.style.height="120px"; input.style.border="none"; input.style.background="none"; input.style.cursor="pointer"; row.appendChild(input);
		    ...
		    var hexField=document.createElement("input"); hexField.type="text"; hexField.value=input.value; hexField.style.fontSize="32px"; hexField.style.padding="6px 10px"; hexField.style.width="220px";
		    ...
		    var prev=document.createElement("div"); prev.style.width="160px"; prev.style.height="120px"; prev.style.borderRadius="12px"; prev.style.boxShadow="inset 0 0 0 2px rgba(255,255,255,.2)"; prev.style.background=input.value; row.appendChild(prev);
		    input.oninput=function(){ hexField.value=input.value; prev.style.background=input.value; };
		    box.appendChild(row);
		    var buttons=document.createElement("div"); buttons.style.display="flex"; buttons.style.gap="16px"; buttons.style.marginTop="10px";
		    var btnReplace=bigBtn("Replace","#1f6feb"), btnAdd=bigBtn("Add as New","#0e8a16"), btnCancel=bigBtn("Cancel","#9b1b1b");
		    function bigBtn(label,bg){ var b=document.createElement("button"); b.textContent=label; b.style.font="900 32px -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif";
		      b.style.padding="10px 18px"; b.style.borderRadius="12px"; b.style.border="none"; b.style.background=bg; b.style.color="#fff"; b.style.cursor="pointer"; return b; }
		    ...
		    mountEl(overlay); colorEditorEl=overlay;
		  ...
		  function buildPanel(){
		    ...
		    panel.style.left="50%"; panel.style.bottom="24px"; panel.style.transform="translateX(-50%)";
		    ...
		    // Upload controls
		    var upWrap=document.createElement("div"); upWrap.style.margin="12px 0 0"; upWrap.style.padding="18px";
		    upWrap.style.borderRadius="16px"; upWrap.style.background="rgba(255,255,255,0.06)";
		    upWrap.style.display="grid"; upWrap.style.gridTemplateColumns="auto auto auto 1fr"; upWrap.style.gap="18px";
		    ...
		    var opSlider=document.createElement("input"); opSlider.type="range"; opSlider.min="0"; opSlider.max="1"; opSlider.step="0.01"; opSlider.value=__uploadOpacity; opSlider.style.width="600px"; opSlider.style.height="48px";
		    ...
		    mountEl(panel); // <<< important: mount into overlay
		    mountEl(fileInput);
		    ...
		    // Tiny HUD toggle button (always visible)
		    var fab=document.createElement("button");
		    fab.textContent="HUD";
		    fab.style.right="16px"; fab.style.top="16px"; fab.style.zIndex="2147483647";
		    fab.style.padding="10px 16px"; fab.style.borderRadius="999px"; fab.style.border="none";
		    fab.style.font="700 16px -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif";
		  ...
		  function makeDraggableFixed(el, handle){
		    ...
		    function move(e){ if(!drag)return; e.preventDefault();
		      ...
		      el.style.left=(startL+dx)+"px"; el.style.top=(startT+dy)+"px"; el.style.transform=""; }
		  ...
		  waitForStage(function(){
		    ...
		    window.addEventListener("resize", function(){ layoutUploadBitmap(); });
		  });
...
lib.properties = {
	...
	opacity: 1.00,
	manifest: [
		{src:"images/pi_atlas_1.png?1758160866561", id:"pi_atlas_1"}
	],
	preloads: []
};
