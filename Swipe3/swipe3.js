...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		// --- NEW: Top Score Global Variables ---
		var topScore = 30000;      // Default top score is 30,000
		var topScoreText;          // Will hold the top score display
		...
		function getBlockSymbol(color) {
		  switch(color) {
		    ...
		    case "white": return new lib.BlockWhite(); // Assuming white piece exists
		...
		function getBlockerSymbol(color) {
		  switch(color) {
		    ...
		    case "white": return new lib.BlockWhiteBLOCKER(); // Assuming white blocker exists
		...
		/*
		  ...
		  - As it finds contiguous groups (with group length ≥ 3), it calculates the reward
		    and collects match messages:
		      • For red, yellow, blue: 4-match → "NICE!"
		      • For white: 3-match → "GREAT!", 4-match → "AMAZING!"
		      • For black: 3-match → "AWESOME!!", 4-match → "INCREDIBLE!"
		*/
		    ...
		    while (k < j) {
		      ...
		      if (groupLength >= 3) {
		         ...
		         if (color === "red" || color === "yellow" || color === "blue") {
		           ...
		           if(groupLength === 4) matchMessages.push("NICE!");
		         } else if (color === "white") {
		           reward = (groupLength === 3 ? 250 : 500);
		           if(groupLength === 3) matchMessages.push("GREAT!");
		           else if(groupLength === 4) matchMessages.push("AMAZING!");
		         } else if (color === "black") {
		           reward = (groupLength === 3 ? 1000 : 2000);
		           if(groupLength === 3) matchMessages.push("AWESOME!!");
		           else if(groupLength === 4) matchMessages.push("INCREDIBLE!");
		         }
		         scoreInc += reward;
		...
		/* --- NEW: showMatchMessage: Display match message above Top Score */
		function showMatchMessage(message) {
		  var matchText = new createjs.Text(message, "bold 130px Arial", "#FFF9F9");
		  matchText.textAlign = "center";
		  matchText.x = (boardCols * cellSize) / 2;
		  // Position it above the top score
		  matchText.y = topScoreText.y - 120;
		  matchText.alpha = 0;
		  stage.addChild(matchText);
		  
		  createjs.Tween.get(matchText)
		    .to({alpha: 1, y: matchText.y - 20}, 500, createjs.Ease.quadOut)
		    .wait(1000)
		    .to({alpha: 0}, 100)
		    .call(function() {
		...
		function init() {
		  ...
		  // --- NEW: Top Score Display ---
		  topScoreText = new createjs.Text("TOP SCORE: " + topScore, "bold 48px Arial", "#FFF9F9");
		  topScoreText.textAlign = "center";
		  topScoreText.x = (boardCols * cellSize) / 2;
		  topScoreText.y = boardRows * cellSize + scoreOffset - 80;
		...
		function animateRowSwipe(rowIndex, direction, onLineComplete) {
		  ...
		  // Display any match messages from this row swipe.
		  if (result.matchMessages && result.matchMessages.length > 0) {
		    result.matchMessages.forEach(function(msg) {
		...
		/* animateScore: Animates score change and updates top score */
		...
		/* showBonusText: Displays bonus score text */
		function showBonusText(points) {
		  var bonusText = new createjs.Text("(+" + points + ")", "bold 48px Arial", "#FFF9F9");
		  bonusText.alpha = 0;
		  bonusText.textAlign = "center";
		  bonusText.x = (boardCols * cellSize) / 2;
		  bonusText.y = scoreText.y + scoreText.getMeasuredHeight() + 50;
		...
		*/
		function addRandomBlockAnimated() {
		  ...
		  if (score < 500) {
		    ...
		    allowedColors = ["red", "yellow", "blue", "white"];
		    ["red", "yellow", "blue"].forEach(function(c){ blockerChanceMapping[c] = 0.3; });
		    blockerChanceMapping["white"] = 0;
		  } else if (score < 4000) {
		    allowedColors = ["red", "yellow", "blue", "white"];
		    allowedColors.forEach(function(c){ blockerChanceMapping[c] = 0.3; });
		  } else if (score < 10000) {
		    allowedColors = ["red", "yellow", "blue", "white", "black"];
		    ["red", "yellow", "blue", "white"].forEach(function(c){ blockerChanceMapping[c] = 0.3; });
