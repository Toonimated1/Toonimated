...
if (reversed == null) { reversed = false; }
	...
	this.instance.setTransform(149.95,137.4,1,1,0,0,0,0,4.7);
	this.instance.filters = [new cjs.ColorMatrixFilter(new cjs.ColorMatrix(-28, 0, 38, 0))];
	this.instance.cache(-142,-129,283,258);
...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		// Global reference for the dithering overlay so it can always be kept on top.
		var ditheringOverlay = null;
		
		// ----- Retro Dithering Global Variables -----
		// The original retro dithering variables remain defined but the filter will no longer be used.
		var retroWidth = 400;
		var retroHeight = 0;
		...
		// ----- OLD Dithering Function (No longer used) -----
		// function applyBayer8x8_32color() { … }
		
		// ----- NEW: Add Dithering Overlay -----
		// Instead of using a per-pixel dithering filter, this function creates an overlay using the library symbol "dithering".
		function addDitheringOverlay() {
		  // Create an instance of the symbol "dithering" from the library.
		  ditheringOverlay = new lib.dithering();
		  ditheringOverlay.name = "dithering"; // Set instance name as required.
		  ditheringOverlay.alpha = 0.5;         // 30% opacity.
		  ditheringOverlay.compositeOperation = "screen"; // Use overlay blending mode.
		  
		  // Position at the top-left corner.
		  ditheringOverlay.x = 0;
		  ditheringOverlay.y = 0;
		  
		  // Optionally scale the image to cover the whole canvas.
		  var canvas = document.getElementById("canvas");
		  var bounds = ditheringOverlay.getBounds();
		  if (bounds) {
		    ditheringOverlay.scaleX = canvas.width / bounds.width;
		    ditheringOverlay.scaleY = canvas.height / bounds.height;
		  }
		  
		  // Add the overlay to the stage.
		  stage.addChild(ditheringOverlay);
		}
		...
		/*
		  ...
		  (Ghosting/trail effect removed.) This function now simply tweens the instance.
		*/
		...
		/*
		  ...
		  - As it finds contiguous groups, it calculates the reward and collects match messages.
		...
		*/
		...
		function init() {
		  ...
		  // Add the dithering overlay so that it covers EVERYTHING.
		  addDitheringOverlay();
		  
		  animateBoardLightUp(function() {
		    addRandomBlockAnimated(); // Initial spawn.
		  });
		  
		  createjs.Ticker.framerate = 30;
		  createjs.Ticker.addEventListener("tick", function(evt) {
		    stage.update();
		    // Ensure the dithering overlay is always on top of everything.
		    if (ditheringOverlay) {
		      stage.setChildIndex(ditheringOverlay, stage.getNumChildren() - 1);
		    }
		  });
		}
