	...
	this.frame_0 = function() {
		...
		/*
		  ...
		  - As it finds contiguous groups (with group length ≥ 3), it calculates the reward
		    and collects match messages as objects with a message text (which may be empty)
		    and a reward value.
		  - NEW: If a group of 4 or more blocks is found, mark the color for a global board clear.
		*/
		var clearColors = []; // Global array for colors that should be cleared from the board
		...
		function computeSlide(line) {
		  ...
		  while (index < line.length) {
		    // If current cell is a blocker, copy it directly.
		    ...
		    // Determine the segment (without blockers)
		    var segStart = index;
		    ...
		    // Collect movable pieces from this segment.
		    var movable = [];
		    ...
		    // Slide movable pieces to the left within the segment.
		    var pos = segStart;
		  ...
		  // Scan contiguous segments for matching.
		  var finalMapping = [];
		  var i = 0;
		...
		/* --- NEW: showMatchMessage ---
		   Displays a match message along with the bonus points.
		   If obj.text is nonempty, the message is shown as "MESSAGE (+reward)",
		   otherwise it shows just "(+reward)".
		   Now the text is positioned directly under the Score.
		*/
		...
		/* --- NEW: animateShakeEffect: Subtle staggered shake for all pieces when no move occurs ---
		   The entire animation per block is set to 200ms with a slight random delay (0–20ms).
		*/
		...
		*/
		...
		/* animateRowSwipe: Animate one row swipe (left/right)
		   Modified to return (via callback) whether the row moved and the reward earned.
		*/
		...
		/* animateColumnSwipe: Animate one column swipe (up/down)
		   Modified to return (via callback) whether the column moved and the reward earned.
		*/
		  ...
		  animateHorizontalSwipe("left", function(moved) {
		    if (moved) {
		      setTimeout(function() {
		        ...
		        // Clear blocks globally for colors with a 4+ block match.
		        clearColors.forEach(function(color) {
		          clearColorFromBoard(color);
		        });
		        clearColors = [];
		        
		        // Spawn new block on opposite edge (right side for left swipe)
